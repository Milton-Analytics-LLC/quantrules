...
class TestObv:
    ...
    def test_missing_close_makes_later_values_unknown(self) -> None:
        """Once a bar's move is unknown, the running OBV total is unknown thereafter."""
        close = series([10.0, 11.0, float("nan"), 10.0, 12.0])
        volume = series([100.0, 200.0, 150.0, 300.0, 250.0])

        result = obv(close, volume)

        assert list(result.iloc[:2]) == [0.0, 200.0]
...
class TestAccumulationDistribution:
    ...
    def test_missing_input_is_unknown_not_zero_flow(self) -> None:
        """A bar with a missing close has unknown flow, so the line is NaN there."""
        high = series([10.0, 10.0])
        low = series([8.0, 8.0])
        close = series([9.0, float("nan")])
        volume = series([100.0, 200.0])
    ...
    def test_missing_bar_makes_later_values_unknown(self) -> None:
        """The cumulative line is unknown for every bar after a missing contributor."""
        high = series([10.0, 10.0, 10.0])
        low = series([8.0, 8.0, 8.0])
        close = series([9.0, float("nan"), 9.0])
        volume = series([100.0, 200.0, 300.0])
    ...
    def test_zero_range_bar_with_missing_close_is_unknown(self) -> None:
        """High == low with a missing close is still an incomplete bar, so it is NaN."""
        high = series([10.0, 10.0])
        low = series([10.0, 8.0])
        close = series([float("nan"), 9.0])
        volume = series([100.0, 200.0])
    ...
    def test_zero_range_bar_contributes_zero_flow(self) -> None:
        """High == low is a real bar with no range, so its flow is zero, not unknown."""
        high = series([10.0, 10.0])
        low = series([10.0, 8.0])
        close = series([10.0, 9.0])
        volume = series([100.0, 200.0])
...
class TestChaikinMoneyFlow:
    ...
    def test_missing_bar_makes_the_window_unknown(self) -> None:
        """A window containing a bar with missing inputs is NaN, not volume-biased."""
        high = series([10.0, 10.0, 10.0])
        low = series([8.0, 8.0, 8.0])
        close = series([9.0, float("nan"), 9.0])
        volume = series([100.0, 200.0, 300.0])
