...
class TestZscore:
    ...
    def test_rejects_ddof_not_below_window(self) -> None:
        with pytest.raises(ConfigurationError, match="ddof"):
            zscore(series([1.0, 2.0, 3.0]), window=2, ddof=2)
...
class TestClip:
    ...
    def test_rejects_a_non_finite_lower_bound(self) -> None:
        with pytest.raises(ConfigurationError, match="lower"):
            clip(series([1.0, 2.0]), lower=float("nan"), upper=1.0)

    def test_rejects_a_non_finite_upper_bound(self) -> None:
        with pytest.raises(ConfigurationError, match="upper"):
            clip(series([1.0, 2.0]), lower=-1.0, upper=float("inf"))
