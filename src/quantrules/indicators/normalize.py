...
from quantrules._validation import ensure_ddof, ensure_finite, ensure_positive_int, ensure_series
...
def zscore(
    ...
    *,
    ...
    return (validated - rolling.mean()) / rolling.std(ddof=degrees)
...
def clip(series: FloatSeries, *, lower: float, upper: float) -> FloatSeries:
    ...
    Raises:
        ConfigurationError: If either bound is not finite, or ``lower`` exceeds
            ``upper``.
    """
    validated = ensure_series(series, "series")
    low = ensure_finite(lower, "lower")
    high = ensure_finite(upper, "upper")
    if low > high:
        message = f"lower ({low}) must not exceed upper ({high})"
        raise ConfigurationError(message)
    return validated.clip(lower=low, upper=high)
