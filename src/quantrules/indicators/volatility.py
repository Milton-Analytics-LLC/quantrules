...
def realized_volatility(
    ...
    *,
    ...
    Raises:
        ConfigurationError: If ``ddof`` is not below ``window``.
        DataValidationError: If any price is not strictly positive (log returns
            are undefined for a zero or negative price).
    """
    validated = ensure_series(price, "price")
    length = ensure_positive_int(window, "window")
    degrees = ensure_ddof(ddof, length)
    yearly = ensure_positive_int(periods_per_year, "periods_per_year")
    if bool((validated <= 0.0).any()):
        message = "price must be strictly positive to take log returns; found a value <= 0"
        raise DataValidationError(message)
    log_returns = _log_returns(validated)
    dispersion = log_returns.rolling(window=length, min_periods=length).std(ddof=degrees)
...
def true_range(high: FloatSeries, low: FloatSeries, close: FloatSeries) -> FloatSeries:
    ...
    ``high - low``. Any later bar missing its high, low, or prior close has an
    unknown range and yields `NaN`.
    ...
    # Propagate NaN: a bar missing its high, low, or prior close has an unknown
    # range. The first bar is the one exception - it has no prior close by
    # definition, not by omission, so it falls back to high - low.
    true_ranges = ranges.max(axis=1, skipna=False)
    first_bar = pd.Series(np.arange(len(highs)) == 0, index=highs.index)
    return cast("FloatSeries", true_ranges.mask(first_bar, high_low))
