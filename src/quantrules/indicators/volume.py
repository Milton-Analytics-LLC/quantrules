...
def _money_flow_volume(
    ...
    # A complete zero-range bar (high == low with a present close) carries no
    # directional information, so force its multiplier to 0. A missing high, low
    # or close leaves the bar unknown (NaN) rather than counted as zero flow.
    multiplier = multiplier.mask((spread == 0.0) & close.notna(), 0.0)
