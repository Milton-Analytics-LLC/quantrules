...
def advance_decline_line(advances: FloatSeries, declines: FloatSeries) -> FloatSeries:
    ...
    Worked example: net advances ``20, 20, -20`` accumulate to ``20, 40, 20``. A
    missing count leaves the running total unknown (`NaN`) from that bar on.
    ...
    # skipna=False so a missing count leaves the running total unknown onward.
    return (advancing - declining).cumsum(skipna=False)
